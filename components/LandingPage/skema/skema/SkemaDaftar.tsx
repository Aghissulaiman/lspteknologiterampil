...
export default function SkemaDaftar() {
  const [searchQuery, setSearchQuery] = useState("");
  const [selectedCategory, setSelectedCategory] = useState("Semua");
  // State untuk melacak kartu mana yang lagi di-hover
        ...
        <div className="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-4 mb-8 bg-card border border-border p-4 rounded-2xl shadow-sm">
          ...
          <div className="relative flex-1">
            ...
            <input
              ...
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full rounded-xl border border-border bg-background pl-10 pr-4 py-2 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-[#ff6600]/50"
            />
          </div>

          {/* Category Tabs (KEMBALI KE KODE AWAL KAMU) */}
          <div className="flex items-center gap-1.5 overflow-x-auto pb-1 md:pb-0">
            {categories.map((cat, idx) => (
              <button
                key={idx}
                ...
                className={`px-3.5 py-1.5 rounded-xl text-xs font-bold whitespace-nowrap transition-all cursor-pointer ${selectedCategory === cat
                    ? "bg-primary text-primary-foreground shadow-sm"
                    : "bg-muted text-muted-foreground hover:bg-[#ff6600] hover:text-white"
                  }`}
              >
                {cat}
              </button>
            ...
            filteredSkema.map((skema, idx) => {
              ...
              return (
                <div
                  ...
                  style={{
                    borderColor: isHovered ? "#ff6600" : undefined,
                  }}
                  className="relative rounded-2xl border border-border bg-card p-6 shadow-sm transition-all duration-300 flex flex-col justify-between cursor-pointer"
                >
                  <div>
                    {/* Top Meta Info */}
                    <div className="flex items-center justify-between gap-2 mb-3">
                      <span
                        style={{
                          color: isHovered ? "#ff6600" : undefined,
                          backgroundColor: isHovered ? "rgba(255, 102, 0, 0.1)" : undefined,
                        }}
                        className="font-mono text-xs font-extrabold text-primary bg-primary/10 px-2.5 py-1 rounded-lg transition-colors"
                      >
                        {skema.code}
                      </span>
                      <span className="text-xs font-semibold text-muted-foreground bg-muted px-2.5 py-1 rounded-lg border border-border">
                    ...
                    {/* Title & Category */}
                    <h3
                      style={{ color: isHovered ? "#ff6600" : undefined }}
                      className="text-xl font-extrabold text-foreground transition-colors"
                    >
                      {skema.title}
                    </h3>
                    <p className="text-xs font-bold text-muted-foreground mt-0.5">
                    ...
                    <div className="mt-4 pt-4 border-t border-border/60">
                      <div className="flex items-center justify-between mb-2">
                        <span className="text-xs font-bold text-foreground flex items-center gap-1.5">
                          <BookOpen
                            style={{ color: isHovered ? "#ff6600" : undefined }}
                            className="h-3.5 w-3.5 text-primary transition-colors"
                          />
                          Unit Kompetensi Utama
                        </span>
                        <span className="text-[11px] font-semibold text-muted-foreground">
                      ...
                      <ul className="space-y-1.5">
                        {skema.popularUnits.map((unit, uIdx) => (
                          <li key={uIdx} className="flex items-center gap-2 text-xs text-muted-foreground">
                            <CheckCircle2
                              style={{ color: isHovered ? "#ff6600" : undefined }}
                              className="h-3.5 w-3.5 text-primary transition-colors shrink-0"
                            />
                            <span className="truncate">{unit}</span>
                          </li>
                        ))}
                  ...
                  <div className="mt-6 pt-4 border-t border-border/60 flex items-center justify-between">
                    ...
                    <a
                      href="#"
                      style={{ color: isHovered ? "#ff6600" : undefined }}
                      className="inline-flex items-center gap-1.5 text-xs font-bold text-primary transition-colors"
                    >
                      <span>Detail Skema & Dokumen</span>
                      <ArrowUpRight
                        style={{ color: isHovered ? "#ff6600" : undefined }}
